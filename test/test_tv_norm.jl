...
g = TV_norm_2D(n; T=T); flag_gpu && (g = g |> gpu)
...
test_grad(fun, y; step=T(1e-3), rtol=T(1e-2))
...
g = TV_norm_3D(n; T=T); flag_gpu && (g = g |> gpu)

# Optimization options
opt = opt_fista(; steplength=T(1/12), niter=100000, tol_x=nothing, nesterov=true)
