function MRIrecon_dTV(contrast, sampling, sideinfo_filename, sideinfo_str, sideinfo, data, groundtruth, options, s_image, alphas, etas, subfolder)
% MRIrecon_dTV
%   MRIrecon_dTV(contrast, sampling, sideinfo_filename, sideinfo_str, sideinfo, data, groundtruth, options, s_image, alphas, etas, subfolder)
% executes the MRI reconstruction with a directional total variation prior. 
...
% -------------------------------------------------------------------------
    ...
    for e = 1 : length(etas)
        prior_options.sideinfo = Vec2D_mult(g,1./ne(n, etas(e)));
    ...
    % save best side info
    prior_options.sideinfo = Vec2D_mult(g,1./ne(n, eta_opt));
    C = 256;
    imwrite((C-1)*(1-Norm_n(prior_options.sideinfo,0,3)), colormap(gray(C)), [filename '_sideinfo_norminv.png']);
    v = prior_options.sideinfo(:,:,1)+1i*prior_options.sideinfo(:,:,2);
    p = Image_complex(v,[],[],[],pi);
    imwrite(p, [filename '_sideinfo.png']);
