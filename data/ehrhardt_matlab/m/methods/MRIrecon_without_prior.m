function MRIrecon_without_prior(contrast, sampling, data, groundtruth, options, s_image, subfolder)
% MRIrecon_without_prior
%   MRIrecon_without_prior(contrast, sampling, data, groundtruth, options, s_image, subfolder)
% executes the MRI reconstruction without a prior. The resuls will be saved
% as files in the folders specified by "options" and if desired in a
% specific "subfolder".
%
% Input:    
%   contrast [string]              
...
% -------------------------------------------------------------------------
    ...
    [~, stats] = MRI2_ADMM(MRI_data, ADMM_options, prior_options);
    u_all = stats.all;

    % save results
    prior_options.name = 'early_stopping';
    ...
    [u_opt, i_opt] = MRIstats_one_param(u_all, gt, 1:ADMM_options.n_iter, 'n iter', filename);
    figure(101); clf; MRI2_ADMM_plot_stats(stats);
    saveas(gcf, [filename '_stats.png']);
    prior_options.prox_options.PC = [];
    save([filename '.mat'], 'u_opt', 'u_all', 'stats', 'ADMM_options', 'sampling', 'contrast', 'prior_options', 'MRI_data');
