function [u, p_nn, stats] = FGP_dTV(data, alpha, n_iter, v, options, p_n)
% FGP_dTV
%   [u, p_nn, stats] = FGP_dTV(data, alpha, n_iter, v, options) runs the fast 
% gradient projection algorithm on the AQPL dual problem. The
...
% -------------------------------------------------------------------------
    ...
    if ~debug % for efficiency
        for n = 1:n_iter
            p_nn = PP(q + factr * G(PC(data + alpha*D(q,data,v)),v));
        ...
        timer = tic;
        fun_0 = obj_fun(zeros(s_data),alpha,data,v);
...
function x = G(x,v)
    x = Gradient2D_forward_constant_unitstep(x,v);
    x = Vec2D_proj_orth(x,v);
end
...
function fun = obj_fun(x, alpha, data, v)
    % data
    res = x - data;
    ...
    f2 = Penalty_dTV(prior_options, x);
